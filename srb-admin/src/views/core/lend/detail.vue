<template>
  ...
  export default {
    ...
    methods: {
        getLendById(){
            lendApi.getLendById(this.$route.params.id).then(res=>{
                this.lendDetail=res.data
                // this.lendDetail.borrower=res.data.borrower
                // this.lendDetail.lend=res.data.lend
            })
        },
        back(){
        ...
        getLendItem(){
          lendItemApi.getLendItemList(this.$route.params.id)
          .then(res=>{
            this.lendItemList=res.data.row
          })
        },
        fetchLendReturnList() {
            lendReturnApi.getList(this.$route.params.id).then(response => {
                this.lendReturnList = response.data.row
            })
        }
    },
  }
